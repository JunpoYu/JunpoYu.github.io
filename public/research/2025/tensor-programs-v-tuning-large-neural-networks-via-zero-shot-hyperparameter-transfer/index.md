...
|                   <span>$\mu$ </span>Transferable                   |            Not <span>$ </span>\mu$Transferable            |                   <span>$\mu$ </span>Transferred Across                    |
| :----------------------------------------------------: | :-----------------------------------------: | :----------------------------------------------------------: |
...
很显然，如果我们不进行缩放，当 <span>$n\to \infty$ </span> 时，那么 <span>$S_n$ </span> 的分布将会发散。或者进行 <span>$\frac{1}{n}$ </span> 缩放，那么 <span>$S_n$ </span> 的方差也会趋向于零，一个均值为零，方差为零的分布是完全没有意义的（或者说）。
...
| :-------: | :----------------------: | :--------------------------------------------: | :--------------------------: |
| Init. Var |   <span>$\frac{1}{fan\_in}$</span>    | <span>$\frac{1}{fan\_in^2}\quad (\frac{1}{fan\_in})$</span> |    <span> $\frac{1}{fran\_in}$</span>     |
|  SGD LR   |   <span>$fan\_out\quad (1) $</span>   |          <span>$\frac{1}{fan\_in}\quad (1)$</span>          |             $1$              |
|  Adam LR  |           $1$            |         <span> $\frac{1}{fan\_in}\quad (1)$ </span>         | <span>$\frac{1}{fan\_in}\quad (1)$ </span>|

其中<span> $fan\_in$ </span> 表示当前神经元的输入连接数，即上一层的神经元数量。<span> $fan\_out$ </span> 表示当前神经元的输出连接数，即下一层神经元的数量。每一个单元格中，左侧表示 $\mu P$ 的配置，右侧使用括号包括的部分表示对应的 SP 配置。对于 bias 而言其 <span> $fan\_in$ </span> 总是 1。

通常在使用中我们也会引入常数因子来参数化<span>  $fan\_in\; \&\; fan\_out$ </span> 。
...
| :--------: | :----------------------: | :--------------------------: | :--------------------------: |
| Init. Var  |   <span>$\frac{1}{fan\_in}$</span>    | <span>$1\quad (\frac{1}{fan\_in})$</span> |     <span>$\frac{1}{fran\_in}$</span>     |
| Multiplier |           $1$            | <span>$\frac{1}{fan\_in}\quad (1)$</span> |             $1$              |
|   SGD LR   |   <span>$fan\_out\quad (1) $ </span>  |      <span>$fan\_in\quad (1)$</span>      |             $1$              |
|  Adam LR   |           $1$            |             $1$              | <span>$\frac{1}{fan\_in}\quad (1)$</span> |

|  Table.9   |          Input weights & all bias          |           Output weights            |        Hidden weights        |
| :--------: | :----------------------------------------: | :---------------------------------: | :--------------------------: |
| Init. Var  | <span>$\frac{1}{fan\_out}\quad\frac{1}{fan\_in}$</span> |    <span>$1\quad (\frac{1}{fan\_in})$</span>     |    <span> $\frac{1}{fran\_in}$</span>     |
| Multiplier |          <span>$\sqrt {fan\_out}\quad1$ </span>          | <span>$\frac{1}{\sqrt{fan\_in}}\quad (1)$ </span>|             $1$              |
|   SGD LR   |           <span> $fan\_out\quad (1) $ </span>           |        <span> $fan\_in\quad (1)$ </span>         |             $1$              |
|  Adam LR   |   <span> $\frac{1}{\sqrt{fan\_out}}\quad(1)$ </span>    | <span>$\frac{1}{\sqrt{fan\_in}}\quad(1)$ </span> | <span> $\frac{1}{fan\_in}\quad (1)$ </span> |
